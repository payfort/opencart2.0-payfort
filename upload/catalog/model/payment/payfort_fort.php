...
class ModelPaymentPayfortFort extends Model {
	public function getMethod($address, $total) {
		$this->language->load('payment/payfort_fort');
            ...
            if ($sadad_enabled){
                $extra_options .= '$("input[value=payfort_fort]").closest("div").after(\'<div class="radio"><label><input type="radio" name="payment_method" value="payfort_fort" id="payfort_sadad"> '.$this->language->get('text_sadad').'</div>\')';
            }
            if ($naps_enabled){
                $extra_options .= ';$("input[value=payfort_fort]").eq(0).closest("div").after(\'<div class="radio"><label><input type="radio" name="payment_method" value="payfort_fort" id="payfort_naps"> '.$this->language->get('text_naps').'</div>\')';
            }
