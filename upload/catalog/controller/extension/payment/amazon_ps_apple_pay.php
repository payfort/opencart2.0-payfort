...
class ControllerExtensionPaymentAmazonPSApplePay extends Controller {
    ...
    public function validate() {
        try {
			...
			$this->amazonpspaymentservices->log('validate Apple Pay: '.$e->getMessage());
	...
	public function init_apple_pay_api( $apple_url ) {
		...
		if (curl_errno($ch)) {
            ...
            $this->amazonpspaymentservices->log('Apple Curl Call error : '.$error_msg);
        }
		curl_close( $ch );
		return $response;
	}
	...
	public function get_cart_data(){
		...
		try {
			...
			$this->amazonpspaymentservices->log( 'APS apple_pay get cart data \n\n' . $e->getMessage() );
			$status = 'error';
		}
	...
	public function validateShippingAddress(){
		try{
			...
			$this->amazonpspaymentservices->log('Apple validate shipping error : '.$e->getMessage());
		}
	...
	public function getshppingMethod($country_info, $zone_info, $postalCode, $city){
		try{
			...
			$this->amazonpspaymentservices->log('Apple shipping error : '.$e->getMessage());
		}
