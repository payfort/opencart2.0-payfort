...
class AmazonPSOrderPayment {
    ...
    public function __construct($registry) {
        ...
        $registry->get('load')->model('account/order');
        $this->model_account_order = $registry->get('model_account_order');
    ...
    public function handleAmazonPSResponse($apsParams = array(), $responseMode = 'online', $integrationType = 'redirection', $tokenization_purchase = false)
    {
        ...
        try {
            ...
            $this->session->data['aps_order_id'] = $this->getOrderId($order);
            $this->amazonpspaymentservices->log('aps_order_id'.$this->session->data['aps_order_id']);
   ...
   private function get_valu_products_data() {
        ...
        $order_products = $this->model_account_order->getOrderProducts($orderId);
    ...
    */
        ...
        if($payment_method == AmazonPSConstant::AMAZON_PS_PAYMENT_METHOD_CC){
            if($this->cart->hasRecurringProducts()){
                ...
                foreach ($this->cart->getRecurringProducts() as $item) {
                   ...
                   $order_recurring_id = $this->model_checkout_recurring->create($item, $orderId, $recurring_description);
                }
            }
        }
    }
    ...
    public function get_apple_order_data() {
        ...
        $orderProducts = $this->model_account_order->getOrderProducts($orderId);
        $orderTotalData = $this->model_account_order->getOrderTotals($orderId);
