...
class ModelExtensionPaymentAmazonPS extends Model {
	...
	public function addEvents(){
		...
		$this->model_extension_event->addEvent('amazon_ps_apple_pay_btn', 'catalog/view/*/template/product/product/after', 'extension/payment/amazon_ps_apple_pay/productBtn');
		$this->model_extension_event->addEvent('amazon_ps_apple_pay_btn', 'catalog/view/*/template/checkout/cart/after', 'extension/payment/amazon_ps_apple_pay/cartBtn');
		$this->model_extension_event->addEvent('amazon_ps', 'catalog/view/*/template/common/success/after', 'extension/payment/amazon_ps/displayPaymentData');
	}
	public function deleteEvents(){
		$this->load->model('extension/event');
		$this->model_extension_event->deleteEvent('amazon_ps_apple_pay_btn');
		$this->model_extension_event->deleteEvent('amazon_ps');
	}
	...
	public function getLanguageVar(){
		$data = [];
		$tabs_var  =  ['tab_general', 'tab_credit_card', 'tab_installments', 'tab_visa_checkout', 'tab_naps', 'tab_knet', 'tab_valu', 'tab_apple_pay', 'tab_cron'];

		$label_var =  ['label_merchant_config', 'label_global_config', 'label_sign_up_url', 'label_host_to_host_url'];
		...
		$button_var = ['button_capture', 'button_void', 'button_refund'];

		$other_var  = ['apple_pay_certificate'];

		$this->load->language('extension/payment/amazon_ps');

		$lan_var = array_merge($tabs_var, $label_var, $text_var, $entry_var, $help_var, $button_var, $other_var);
	...
	public function doCaptureVoid($orderId, $amount, $command_type, $paymentMethod){
		...
		try {
			...
			$access_code = $this->config->get('amazon_ps_access_code');
			$signature_type = 'regular';
			if($paymentMethod == AmazonPSConstant::AMAZON_PS_PAYMENT_METHOD_APPLE_PAY){
				$access_code = $this->config->get('amazon_ps_apple_pay_access_code');
				$signature_type = 'apple_pay';
			}

			$gateway_params  = array(
				'merchant_identifier' => $this->config->get('amazon_ps_merchant_identifier'),
    ...
    {
        if ($this->registry->get('config')->get('amazon_ps_sandbox_mode')) {
            return true;
        }
        return false;
    }
    ...
    public function getRequestShaPhrase(){
        return $this->registry->get('config')->get('amazon_ps_request_sha_phrase');
    }

    public function getResponseShaPhrase(){
        return $this->registry->get('config')->get('amazon_ps_response_sha_phrase');
    }

    public function getShaType(){
        return $this->registry->get('config')->get('amazon_ps_sha_type');
    }

    public function getApplePayShaType(){
        return $this->registry->get('config')->get('amazon_ps_apple_pay_sha_type');
    } 

    public function getApplePayRequestShaPhrase(){
        return $this->registry->get('config')->get('amazon_ps_apple_pay_request_sha_phrase');
    }

    public function getApplePayResponseShaPhrase(){
        return $this->registry->get('config')->get('amazon_ps_apple_pay_response_sha_phrase');
    }
    ...
    {
        if ($this->registry->get('config')->get('amazon_ps_debug')) {
            return true;
        }
        return false;
    }
    ...
    {
        $gateway_currency = $this->registry->get('config')->get('amazon_ps_gateway_currency');
        $new_amount       = 0;
    ...
    public function convertedToGatewayAmount($amount, $currency_code, $currency_value)
    {
        ...
        if ( 0 !== $decimal_points ) {
            ...
            $this->log( 'APS amount before int \n\n' . $amount);
            $amount = (int)$amount;
            $this->log( 'APS amount after int \n\n' . $amount);
        }
        return $amount;
    }
